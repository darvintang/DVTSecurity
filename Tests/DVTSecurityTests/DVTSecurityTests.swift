@testable import DVTSecurity
import XCTest

final class DVTSecurityTests: XCTestCase {
    let strPuk = """
        -----BEGIN PUBLIC KEY-----
        MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDA7MgpTUMWLvAShQqvEFjmdvC0
        NOMkzCLi1iodZJpWeAzo14GSSXlQtCatjeJBI6G9b0tj4Kdv02c7kCjcOehPL7xy
        m2y06F23konbmJVq3KR1jJS8Xx5OjC40vUJcDiWqw0/ScvYLpD+OQyUiVfwDNttR
        FI3w2Zm+9PPOz4SEYQIDAQAB
        -----END PUBLIC KEY-----
        """

    let strPrk = """
        -----BEGIN PRIVATE KEY-----
        
        -----END PRIVATE KEY-----
        """

    func testExample() throws {
        let encryptString = "你好"
        let encrypt = try? encryptString.rsa.encrypt(self.strPuk)
        let decrypt = try? encrypt?.rsa.decrypt(self.strPrk)

        print("encrypt:", encrypt as Any, "\ndecrypt:", decrypt as Any)
        XCTAssertEqual(decrypt, encryptString)
        let signed = "123"
        do {
            let signature = try signed.rsa.signed(self.strPrk)
            print(signature)
            let res = try signed.rsa.verify(self.strPuk, signature: signature)
            XCTAssertTrue(res)
        } catch {
            print(error)
        }
    }

    func testAES() {
        guard let message = "secret message".data(using: .utf8) else {
            return
        }
        let key = "5TpycbI2z61Oa3gi"
        do {
            let aes = try AES(key: key)
            let result = try aes.encrypt(message)
            let decrypted = try aes.decrypt(result)
            print(String(data: decrypted, encoding: .utf8) ?? "错误")
            XCTAssertTrue(decrypted == message)
        } catch {
            print(error)
        }
    }
}
